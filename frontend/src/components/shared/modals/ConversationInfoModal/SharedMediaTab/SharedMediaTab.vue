...
import type { IContact, IConversation, IMessage } from "@src/utils/types";
import { computed } from "vue";

import { hasAttachments } from "@src/utils/utils";
  ...
  <div>
    ...
    <ScrollBox class="overflow-y-scroll max-h-55.5">
      <div v-if="attachmentMessages.length > 0" v-for="(message, index) in attachmentMessages" :key="index">
        <MediaItem v-for="(attachment, index) in message.attachments" :attachment="attachment" :date="message.date"
          :key="index" />
      </div>
