...
import type { IContact, IConversation } from "@src/utils/types";

import { computed, ref } from "vue";

import { getAvatar, getName, getOddContact } from "@src/utils/utils";
  ...
  <div>
      ...
      <div class="flex">
        ...
        <div class="mr-5">
          <button @click="openImageViewer = true" class="outline-none" aria-label="view avatar">
            ...
            }" class="w-9.5 h-9.5 rounded-full bg-cover bg-center"></div>
          </button>
        </div>
        ...
        <div class="w-full flex justify-between">
          ...
          <IconButton title="edit group" v-if="['group', 'broadcast'].includes(conversation.type)"
    ...
    <div class="w-full py-5 border-t border-gray-100 dark:border-gray-700">
      ...
      <div v-if="conversation.type === 'couple' || props.contact" class="flex px-5 pb-5 items-center">
        <IconAndText :icon="AtSymbolIcon" :title="getOddContact(props.conversation)?.email" />
      </div>
      ...
      " class="px-5 flex items-center pb-5">
        <IconAndText :icon="UserIcon" title="members" link chevron @click="
          $emit('active-page-change', {
            tabName: 'members',
            animationName: 'slide-left',
          })
      ...
      <div class="px-5 pt-5 flex items-center">
        <IconAndText :icon="ShareIcon" title="shared media" link chevron @click="
          $emit('active-page-change', {
