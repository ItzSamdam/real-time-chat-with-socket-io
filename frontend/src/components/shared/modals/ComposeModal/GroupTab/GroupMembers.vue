...
<template>
  <div>
    ...
    <ScrollBox class="overflow-y-scroll max-h-50 mb-5">
      <ContactItem v-if="store.status === 'success' && !store.delayLoading && store.user"
        v-for="(contact, index) in store.user.contacts" @click="handleSelectedContactsChange(contact)"
        :contact="contact" :active="isContactSelected(contact)" :key="index">
