...
<template>
    <!--info button-->
    <button v-if="props.link" class="group w-full flex items-center outline-none" @click="
        $emit('active-page-change', {
            tabName: 'members',
            animationName: 'slide-left',
        })
        ">
        <component :is="props.icon" class="w-[1.25rem] h-[1.25rem] mr-6 transition-all duration-200"
            :class="colorClasses" />
        ...
        <ChevronRightIcon v-if="props.chevron" class="w-[1.25rem] h-[1.25rem] duration-200" :class="colorClasses" />
    </button>
