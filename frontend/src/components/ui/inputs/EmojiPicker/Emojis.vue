...
import type { IEmoji } from "@src/utils/types";
...
<template>
  <ScrollBox class="w-full max-h-68.5 overflow-y-scroll">
    <div v-for="[name, group] in Object.entries(filteredEmojis)" class="mb-6">
      <!--Group title-->
      <p class="heading-2 text-black/70 dark:text-white/70 mb-4 dark:text-white">
        {{ name }}
      </p>
      <div class="flex flex-wrap justify-start">
        <!--Emojis-->
        <div v-for="emoji in group">
          <IconButton v-if="emoji && emoji.r" class="ic-btn-ghost-gray w-7.5 h-7.5 mr-1" :title="emoji.n[0]"
