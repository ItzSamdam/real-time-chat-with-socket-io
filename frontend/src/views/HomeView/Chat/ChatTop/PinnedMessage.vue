...
<template>
  <SlideTransition animation="shelf-down">
    <div
      ...
      <div class="flex">
        <!--hide pinned Message-->
        <IconButton title="hide pinned message" aria-label="hide pinned message" @click="handleHidePinnedMessage"
          class="ic-btn-ghost-primary w-7 h-7" :class="{
            'mr-3':
        ...
        <IconButton v-if="
          store.user && activeConversation?.admins?.includes(store.user.id)
        " @click="handleRemovePinnedMessage" class="ic-btn-ghost-primary w-7 h-7" title="close pinned message"
