  ...
  <div class="w-full">
    ...
    <div class="h-auto min-h-21 p-5 flex items-end" v-if="store.status !== 'loading'"
      ...
      <div class="grow md:mr-5 xs:mr-4 self-end" v-if="!recording">
        <div class="relative">
          <Textarea class="max-h-[5rem] pr-12.5 resize-none scrollbar-hidden"
            @value-changed="(newValue: string) => (value = newValue)" @input="handleSetDraft" :value="value" auto-resize
      ...
      <div class="min-h-[2.75rem] flex">
        ...
        <!--send message button-->
        <IconButton v-if="!recording" class="ic-btn-contained-primary w-7 h-7 active:scale-110" title="send message"
