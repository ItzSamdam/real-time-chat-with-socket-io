...
import { ICall } from "@src/utils/types";
import { getCallName, getOtherMembers } from "@src/utils/utils";
...
<template>
  <div>
    <component :is="props.active ? 'div' : 'button'" :aria-label="'voice call with ' + getCallName(props.call, true)"
      ...
      class="w-full h-23 px-5 py-6 mb-3 flex rounded focus:outline-none transition duration-500 ease-out" :class="props.active
        ...
        <div class="w-full">
          ...
          <div class="flex items-start">
            <div class="grow mb-4 text-start">
              <button v-if="props.active" class="block" @click="props.openVoiceCallModal">
